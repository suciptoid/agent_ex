      ...
      <div class="flex h-full min-h-0 flex-col gap-6 p-4 sm:p-6 lg:p-8">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div
            ...
            class="group relative overflow-hidden rounded-xl border border-border bg-card p-5 shadow-sm transition-all hover:shadow-md hover:border-primary/20"
          >
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="flex size-11 items-center justify-center rounded-xl bg-primary/10 text-primary transition-transform group-hover:scale-110">
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="space-y-1">
                ...
                <p
                  id="dashboard-stat-agents-value"
                  class="text-3xl font-semibold tracking-tight text-foreground"
                >
                  {@agents_count}
                </p>
        ...
        <div class="grid gap-6 xl:grid-cols-2">
          ...
          >
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div class="space-y-1">
                <h2 class="text-base font-semibold text-foreground">Recent conversations</h2>
            ...
            <div class="mt-5 space-y-2">
              <%= if @recent_chat_rooms == [] do %>
                <div
                  ...
                  class="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/30 p-8 text-center text-sm text-muted-foreground"
                >
                  <.icon name="hero-chat-bubble-left-right" class="mb-3 size-8 opacity-40" />
                  <p class="font-medium text-foreground">
                    {recent_chats_empty_title(@providers_count, @agents_count)}
                  </p>
                  <p class="mt-1.5">{recent_chats_empty_copy(@providers_count, @agents_count)}</p>
                </div>
              <% else %>
                <.link
                  ...
                  class="group flex items-center justify-between gap-4 rounded-xl border border-border/70 bg-background/60 px-4 py-3 transition hover:border-primary/30 hover:bg-accent/30"
                >
                  <div class="min-w-0">
                  ...
                  <span class="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground transition group-hover:bg-primary/10 group-hover:text-primary">
          ...
          >
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div class="space-y-1">
                <h2 class="text-base font-semibold text-foreground">Recently added agents</h2>
                <p class="text-sm text-muted-foreground">
            ...
            <div class="mt-5 space-y-2">
              <%= if @recent_agents == [] do %>
                <div
                  ...
                  class="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/30 p-8 text-center text-sm text-muted-foreground"
                >
                  <.icon name="hero-cpu-chip" class="mb-3 size-8 opacity-40" />
                  <p class="font-medium text-foreground">
                    {recent_agents_empty_title(@providers_count)}
                  </p>
                  <p class="mt-1.5">{recent_agents_empty_copy(@providers_count)}</p>
                </div>
              <% else %>
                <div
                  :for={agent <- @recent_agents}
                  id={"recent-agent-#{agent.id}"}
                  class="group rounded-xl border border-border/70 bg-background/60 px-4 py-3 transition hover:border-primary/30 hover:bg-accent/30"
                >
                  <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="inline-flex items-center gap-2 rounded-full border border-border bg-muted/50 px-2.5 py-1 text-xs font-medium text-foreground">
